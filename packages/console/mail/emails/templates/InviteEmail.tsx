...
export const InviteEmail = ({
  ...
  return (
    <Html lang="en">
      <Head>
        <Title>{`${Brand.name} — ${messagePlain}`}</Title>
      </Head>
            ...
            <Row>
              <Column>
                <A href={`${CONSOLE_URL}zen`}>
                  <Img height="32" alt={`${Brand.name} Logo`} src={`${assetsUrl}/logo.png`} />
                </A>
              </Column>
            </Row>

            <Section style={{ padding: `${unit * 2}px 0 0 0` }}>
              <Text style={headingText}>Join your team's {Brand.name} workspace</Text>
              <Text style={contentText}>
                You have been invited by <Span style={contentHighlightText}>{inviter}</Span> to join the{" "}
                <Span style={contentHighlightText}>{workspaceName}</Span> workspace on {Brand.name}.
              </Text>
            </Section>
